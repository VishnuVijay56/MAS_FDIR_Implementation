    ...
    for inner_i in tqdm(range(n_admm), desc="ADMM Loop", leave=False):
        ...
        for agent_id, agent in enumerate(agents):
            ...
            if prob1.status != cp.OPTIMAL:
                print("\nERROR Problem 1: Optimization problem not solved optimally @ (%d, %d, %d)" % (inner_i, outer_i, agent_id))
        ...
        # TODO: Could implement from SK's paper, but not needed for this implementation
        ...
        for agent_id, agent in enumerate(agents):
            ...
            if prob2.status != cp.OPTIMAL:
                print("\nERROR Problem 2: Optimization problem not solved optimally @ (%d, %d, %d)" % (inner_i, outer_i, agent_id))
